...
- **Rationale**: Research interactions are rarely one-off. DailyDialog provides multi-turn trajectories that allow us to test if the system can follow shift in logic (e.g., moving from a general topic to a specific technical question).
...
- **`memory_engine.py`**: The heart of the system. Implements the `UltimateMemoryEngine` class, managing the STM/LTM lifecycle and adaptive weighting.
- **`rag_pipeline.py`**: Orchestrates the dual-retrieval track (Internal Personal vs. External Research) and synthesizes the final grounding response.
- **`data_loader.py`**: Handles asynchronous downloading of Kaggle research datasets and ensures type-safe dataframe processing.
- **`eval_engine.py`**: The laboratory module. Calculates the NDCG and Hallucination metrics for scientific reporting.
- **`main.py`**: The API Gateway. Provides a full FastAPI interface and hosts the static dashboard for non-technical stakeholders.
...
Introduction of a **Saliency Decay Algorithm** that autonomously "forgets" low-utility data points to optimize vector-space density.

### Phase 7: Collaborative Memory 
Implementing a privacy-preserved **Federated Memory Layer** where multiple C-RAG instances can share anonymized research insights.
